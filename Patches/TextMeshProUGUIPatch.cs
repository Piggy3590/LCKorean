...
namespace LCKR.Patches
{
    ...
    internal class TextMeshProUGUIPatch
    {
        ...
        {
            if (SceneManager.GetActiveScene().name == "ColdOpen1" || SceneManager.GetActiveScene().name == "ColdOpen2"
                                                                  || SceneManager.GetActiveScene().name == "InitScene")
            {
                __instance.text = TranslationManager.ReplaceArrayTextAll(__instance.text, "ColdOpen");
            }
            else if (SceneManager.GetActiveScene().name == "MainMenu")
            {
                ...
                __instance.text = __instance.text.Replace("This update introduces the mineshaft, a new creature, and new scrap to collect.", "이번 업데이트에서는 광산 갱도, 새로운 생명체, 그리고 수집할 수 있는 새로운 폐품이 추가되었습니다.");
                __instance.text = __instance.text.Replace("V73 is a security patch. However, additionally, 7-Dine is now home to unique scrap.", "V73은 보안 패치입니다. 추가적으로, 7-다인에서 이제 독특한 폐품을 획득할 수 있습니다.");
                __instance.text = __instance.text.Replace("V80 includes a full rework of March, 3 new creatures, and numerous misc. gameplay improvements and settings, as well as overhauls to increase performance.",
                                    "V80에는 머치의 전반적인 재작업, 3개의 새로운 생명체, 다양한 게임플레이 개선 사항 및 설정 변경, 그리고 성능 향상을 위한 대대적인 개편이 포함되어 있습니다.");
                __instance.text = __instance.text.Replace("The next update will include more creatures and scrap.", "다음 업데이트에서 더 많은 생명체와 폐품이 추가될 예정입니다.");
                ...
                __instance.text = __instance.text.Replace(
                    "Thanks to PuffoThePufferfish and Blueray901165 for helping shape this game. They were the closest with it through the entire process, helping me test and improve it every step of the way.",
                ...
                __instance.text = __instance.text.Replace("Thanks to Scoops, a member of the community, for her perfect recreation of the game's stylization shader which allowed me to add performant grass and other features.", "커뮤니티 회원인 Scoops 님께 감사드립니다. 게임의 스타일화 셰이더를 완벽하게 재현해 주신 덕분에 성능이 뛰어난 잔디와 기타 기능들을 추가할 수 있었습니다.");
				__instance.text = __instance.text.Replace("Thanks to Taaamas (@taaamas.bsky.social) for their pixel art of the game's desktop icon which they generously allowed me to use!", "게임의 바탕화면 아이콘을 픽셀 아트로 제작해 주시고, 사용하도록 허락해 주신 Taaamas(@taaamas.bsky.social) 님께 감사드립니다!");
                ...
                __instance.text = __instance.text.Replace(
                    ...
                    "호스트가 수정된 버전의 Lethal Company를 사용하고 있는 것으로 감지되었습니다. 의도하지 않은 현상이 발생할 수 있습니다. (모드 사용 시에는 주의가 필요하며, 공식적으로 지원되지 않습니다.)");
            }
            ...
            switch (__instance.text)
            {
                ...
                case "Version 60 is here!":
                    ...
                    break;
				case "Welcome to Version 80!":
					__instance.text = "버전 80에 오신 것을 환영합니다!";
					break;
                ...
                //테스트
                case "Toggle Sprint":
                    __instance.text = "달리기 전환";
                    break;
                case "Head Bobbing":
                    __instance.text = "화면 흔들림";
                    break;
                case "Flip Screen Horizontally":
                    __instance.text = "화면 좌우 반전";
                    break;
                case "GRAPHICS":
                    __instance.text = "그래픽";
                    break;
                
                case "Terrain / Grass Detail":
                    __instance.text = "지형 / 잔디 품질";
                    break;
                case "Motion Blur":
                    __instance.text = "모션 블러";
                    break;
                case "Pixel Resolution":
                    __instance.text = "픽셀 해상도";
                    break;
                case "Indirect lighting":
                    __instance.text = "간접 조명";
                    break;
                ...
                case "With challenge moon":
                    ...
                    break;
                case "With detected mods":
                    __instance.text = "모드 서버 포함";
                    break;
                case "WARNING: Modifying Lethal Company is not recommended.":
                    __instance.text = "경고: Lethal Company를 수정하는 것은 권장되지 않습니다.";
                    break;
                case "CHALLENGE MOON":
            ...
            if (!TranslationManager.GetArrayTranslation("HUD", __instance.text).IsNullOrWhiteSpace())
                __instance.text = TranslationManager.GetArrayTranslation("HUD", __instance.text);
        }
    }
}
