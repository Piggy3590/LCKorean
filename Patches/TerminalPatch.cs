...
namespace LCKR.Patches
{
    ...
    internal class TerminalPatch
    {
        ...
        private static string confirmString;
        private static string denyString;
        ...
        {
            confirmString = TranslationManager.GetArrayTranslation("Terminal", "확인");
            denyString = TranslationManager.GetArrayTranslation("Terminal", "취소");
        ...
        */
        ...
        {
            if (___enemyFiles != null)
            {
                foreach (TerminalNode node in ___enemyFiles)
                {
                    if (node == null)
                        continue;
                    if (!TranslationManager.GetArrayTranslation("Terminal", node.name).IsNullOrWhiteSpace())
                    {
                        node.creatureName = TranslationManager.GetArrayTranslation("Terminal", node.name);
                    }
                    
                    if (!TranslationManager.GetArrayTranslation("Terminal", node.name, 1).IsNullOrWhiteSpace())
                    {
            ...
            if (terminalNodes != null && terminalNodes.allKeywords != null)
            {
                            ...
                            foreach (CompatibleNoun noun in keyword.compatibleNouns)
                            {
                                switch (noun.result.name)
                                {
                                    ...
                                    case "buyProFlashlight1":
                                        ...
                                        noun.result.terminalOptions[0].noun.word = confirmString;
                                        noun.result.terminalOptions[1].result.displayText = "주문을 취소했습니다.\n\n";
                                        noun.result.terminalOptions[1].noun.word = denyString;
                                        break;
                                    case "buyFlash":
                        ...
                        case "route":
                            foreach (CompatibleNoun noun in keyword.compatibleNouns)
                            {
                                ...
                                if (noun.result.displayText.Contains("The Company is buying"))
                                {
                                    ...
                                    noun.result.displayText = noun.result.displayText.Replace("Please CONFIRM or DENY.", confirmString.ToUpper() + " 또는 " + denyString.ToUpper() + "을(를) 입력하세요.");
